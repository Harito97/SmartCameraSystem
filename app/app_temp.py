...
class App:
    ...
    def same_object(self, img:np.ndarray, reid_picture:dict, threshold=0.8):
        ...
        if len(result) != 0:
            data_key = max(result, key=result.get)

            self.reid_dict[data_key].add(img_feature)
            reid_picture[data_key].add(App.resize_image(img))
        return reid_picture
        ...
        while cap1.isOpened() and cap2.isOpened():
            ...
            if i % 24 == 0:
                ...
                for per1 in result1_detect[0]:
                    reid_picture = self.same_object(per1, reid_picture, 0.4)
                    
                for per2 in result2_detect[0]:
                    reid_picture = self.same_object(per2, reid_picture, 0.4)
